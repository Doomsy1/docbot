...
def _lang_label(index_or_scope: DocsIndex | ScopeResult) -> str:
    """Return a human-readable language label from a DocsIndex or ScopeResult."""
    langs = getattr(index_or_scope, "languages", [])
    return ", ".join(langs) if langs else "software"
...
_SCOPE_DOC_SYSTEM = """\
You are a technical writer producing module documentation for a \
{languages} repository. Write clear, thorough markdown based on the \
structured data. Never invent features not evidenced in the data. Include \
file and line references where relevant."""

_SCOPE_DOC_PROMPT = """\
Write comprehensive documentation for this module/scope of a {languages} repository.
...
_README_SYSTEM = """\
You are a technical writer producing a README for a {languages} repository. \
...
_README_PROMPT = """\
Generate a README.md for this {languages} repository.
...
_ARCH_SYSTEM = """\
You are a software architect writing an architecture overview for a \
{languages} repository. Base your analysis only on the structured data \
provided."""

_ARCH_PROMPT = """\
Write an architecture overview document (markdown) for this {languages} repository.
...
def _render_scope_md_template(scope: ScopeResult) -> str:
    ...
    if scope.languages:
        lines.append(f"**Languages:** {', '.join(scope.languages)}\n")
...
def _render_readme_template(index: DocsIndex) -> str:
    ...
    if index.languages:
        lines.append(f"**Languages:** {', '.join(index.languages)}\n")
    lines.append("## Scopes\n")
...
def _render_architecture_template(index: DocsIndex) -> str:
    ...
    for s in index.scopes:
        n = len(s.paths)
        total_files += n
        langs = f" [{', '.join(s.languages)}]" if s.languages else ""
        lines.append(f"- **{s.title}** (`{s.scope_id}`){langs} -- {n} file(s)")
    lines.append(f"\nTotal source files covered: {total_files}\n")
    if index.entrypoints:
        lines.append("## Entrypoints\n")
        for ep in index.entrypoints:
            lines.append(f"- `{ep}`")
...
def _render_index_html(index: DocsIndex) -> str:
    ...
    languages_html = ", ".join(_h(l) for l in index.languages) if index.languages else "N/A"
...
Languages: <strong>{languages_html}</strong><br>
...
<h2>Scopes</h2>
<table>
<tr><th>Scope</th><th>Languages</th><th>Files</th><th>Public API</th><th>Env vars</th><th>Errors raised</th><th>Status</th></tr>
{scope_rows}
</table>
...
def _build_scope_summaries(index: DocsIndex) -> str:
    ...
    for s in index.scopes:
        status = "[FAILED]" if s.error else ""
        langs = f" [{', '.join(s.languages)}]" if s.languages else ""
        parts.append(f"- {s.title} ({s.scope_id}, {len(s.paths)} files){langs} {status}")
        if s.summary:
...
def _build_scope_details(index: DocsIndex) -> str:
    ...
    for s in index.scopes:
        langs = f" [{', '.join(s.languages)}]" if s.languages else ""
        parts.append(f"### {s.title} ({s.scope_id}){langs}")
...
async def _generate_scope_doc_llm(scope: ScopeResult, llm_client: object) -> str:
    ...
    return await llm_client.ask(prompt, system=_SCOPE_DOC_SYSTEM.format(languages=languages))
...
async def _generate_readme_llm(index: DocsIndex, llm_client: object) -> str:
    ...
    return await llm_client.ask(prompt, system=_README_SYSTEM.format(languages=languages))
...
async def _generate_architecture_llm(index: DocsIndex, llm_client: object) -> str:
    ...
    return await llm_client.ask(prompt, system=_ARCH_SYSTEM.format(languages=languages))
