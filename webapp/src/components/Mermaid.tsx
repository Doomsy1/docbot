...
// Remove orphaned mermaid error elements that the library injects into document.body
function cleanupMermaidErrors(id: string) {
  // Remove the temp container mermaid creates with our render ID
  const el = document.getElementById(id);
  if (el) el.remove();
  // Also remove the accessibility wrapper mermaid may leave behind
  const dmId = 'd' + id;
  ...
  useEffect(() => {
    async function renderChart() {
        ...
        try {
            ...
            // Clean up any DOM nodes mermaid left behind on failure
